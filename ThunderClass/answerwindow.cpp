...
void AnswerWindow::on_btnSubmit_clicked()
{
    ...
    if (m_bIsRecvAll)//如果发送给全班选择题
    {
        if (ui->ckbOptA->isChecked() || ui->ckbOptB->isChecked() || ui->ckbOptC->isChecked() || ui->ckbOptD->isChecked())//有选择的内容
        {
            //发送结束则清空输入
