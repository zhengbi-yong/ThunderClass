...
public:
    ...
    //第2参数，范静涛添加，表示此Connection是Server创建的第几个，最早创建的是0号Connections对象
    //第3参数，范静涛修改，pair是stl中的一种类型，first表示当前Connection的index，second表示消息。此参数是一个pair的队列的引用，来自Server
    ...
    //范静涛增加, 用于实现收消息队列的有序插入和删除
    static mutex RecvMsgMutex;

private:
    //来自于Server提供的、已经连上的SocketClient（不需深入理解，它就是一个连接的标识，和身份证一样），保存在此私有数据成员
    SOCKET SocketClient;
    //表示是否处于与客户端的连接状态，做成私有是为了外部仅能通过公有IsConnected函数读到，但不可改变此成员的值。体现面型对象的封装概念，隐藏内部机制
    bool m_bIsConnected;
