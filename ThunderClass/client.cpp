...
/*构造函数，仅用于初始化通信线程句柄，socket对象，和当前连接状态。*/
...
/*析构函数，断开可能存在的连接，清除收发消息队列。*/
Client::~Client(){
    DisConnect();
    Clear();
}

/*连接至特定IP地址和端口。IP，入口参数，代表服务端IP地址的字符串类型；Port，入口参数，
 *代表端口号的无符号整型。*/
...
/*获取并删除接收队列中最早收到的消息。aMessage,出口参数，如接收队列中有消息，则用最早收到的消息向此参数赋值*/
...
/*通信线程函数。lpParameter，入口参数，代表线程所属的Client。*/
